...
void setBrightness(uint8_t percent) {
  ...
  const uint8_t brightness = static_cast<uint8_t>((static_cast<uint16_t>(percent) * 255U) / 100U);
  sendCommand(0x51, &brightness, 1);
}
...
void co5300SetBrightnessPercent(uint8_t percent) { setBrightness(percent); }

void co5300Sleep() { co5300SetBacklight(false); }

void co5300Wake() { co5300SetBacklight(true); }
